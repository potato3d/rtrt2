...
RawKdTree* TriangleTreeBuilder::buildTree( rt::Geometry* geometry )
{
	...
	// Reset triangle data
	rt::vectorExactResize( _triangleSides, triangleCount );
	rt::vectorExactResize( _triangleBoxes, triangleCount );
	...
	// Cleanup
	rt::vectorFreeMemory( _triangleSides );
	rt::vectorFreeMemory( _triangleBoxes );
	rt::vectorFreeMemory( _events[0] );
	rt::vectorFreeMemory( _events[1] );
	rt::vectorFreeMemory( _events[2] );
...
RawKdNode* TriangleTreeBuilder::recursiveBuild( const RawKdNode::Elements& triangles, const rt::Aabb& bbox, uint32 treeDepth )
{
	...
	rt::vectorFreeMemory( leftTriangles );
	RawKdNode* right = recursiveBuild( rightTriangles, rightBox, treeDepth + 1 );
	rt::vectorFreeMemory( rightTriangles );
