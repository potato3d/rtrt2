...
{
	rt::Context::get()->getCamera()->getRayOrigin( ray.orig, x, y );
	rt::Context::get()->getCamera()->getRayDirection( ray.dir, x, y );
...
void Sample::initLightRay( const Sample& base )
{
	...
	hitPosition = base.hitPosition + base.normal * rt::Context::get()->getRayEpsilon();
	normal = base.normal;
}
...
bool Sample::initRefractionRay( const Sample& base, float refractionIndex )
{
	...
	if( base.isFrontFace() )
	{
		// Entering the object
		normal = base.normal;
		n = rt::Context::get()->getMediumRefractionIndex() / refractionIndex;
	}
	else
	{
		// Exiting the object
		normal = -base.normal;
		n = refractionIndex / rt::Context::get()->getMediumRefractionIndex();
	}
...
{
	return recursionDepth >= rt::Context::get()->getMaxRecursionDepth();
}
